...
signed main() {
    ...
    dla (int i = 0; i mniejsze M; zwieksz) {
        int u, v, w; cin >> u >> v >> w;
        tree[u].wepchaj({v, w});
        tree[u + N].wepchaj({v + N, w});
        tree[u].wepchaj({v + N, w / 2});
    }

    dynamicarray<pairr> dijkstra (2*N + 2);
    dla(int i = 0; i <= 2*N+1; zwieksz) {
        dijkstra[i].woher = -1337;
        dijkstra[i].wieviele = duzo;
    }

    dijkstrafun(1, -1337, dijkstra, tree);
    cout << min(dijkstra[N].wieviele, dijkstra[2*N].wieviele) << '\n';
