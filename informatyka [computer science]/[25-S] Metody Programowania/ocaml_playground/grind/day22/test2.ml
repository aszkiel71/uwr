...
let rec eval e env stk =
    ...
    | Ref(e) -> id := !id + 1; bind (eval e env stk) (fun (v1, s1) ->
                            return(VRef !id, (!id, v1) :: stk))
    | Deref(e) -> bind (eval e env stk) (fun (v1, s1) ->
                        | VRef a -> (lookup_env a stk, stk)
                        | _ -> failwith "expected reference")
    | Assign(e1, e2) -> bind (eval e1 env stk) (fun (v1, s1) ->
                        bind (eval e2 env s1) (fun (v2, s2) ->
                        match v1 with
                        | VRef a -> return (VUnit, (a, v2) :: (List.remove_assoc a s2))
                        | _ -> failwith "expected reference"))